...
또한, 람다 표현식은 메소드의 매개변수로 전달될 수도 있고, 메소드의 결괏값으로 반환될 수도 있다.
...
다음 예제는 iterate() 메소드를 이용하여 짝수만으로 이루어진 무한 스트림을 생성하는 예제이다.
...
Stream API에서 사용할 수 있는 대표적인 중개 연산과 그에 따른 메소드는 다음과 같다.
...
Stream API에서 사용할 수 있는 대표적인 최종 연산과 그에 따른 메소드는 다음과 같다.
...
위의 예제처럼 인수로 초깃값을 전달하는 reduce() 메소드의 반환 타입은 Optional<T>가 아닌 T 타입이다.
...
위의 예제에서 볼 수 있듯이 두 메소드의 결과는 같게 출력된다.
